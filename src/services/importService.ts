...
export const importService = {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          // Sanitización inicial de toda la data recibida del Excel
          const sanitizedData = jsonData.map(row => {
            const newRow: any = {};
            for (const key in row) {
              // Solo sanitizamos si es string, para no dañar números o fechas
              newRow[key] = typeof row[key] === 'string'
                ? security.sanitizeString(row[key])
                : row[key];
            }
            return newRow;
          });

          const result = await importService.uploadDataInBatches(sanitizedData, onProgress);
          resolve(result);
        } catch (error) {
      ...
      reader.onerror = (error) => {
        logger.error('Error de lectura de archivo', error);
        reject(error);
      };
  ...
  uploadDataInBatches: async (rows: any[], onProgress?: (p: number) => void) => {
    logger.info(`Iniciando carga masiva segura: ${rows.length} filas.`);
    ...
    try {
      ...
      // Insertar asignaciones en trozos para evitar límites de payload y timeouts
      // Con 10k filas, el chunk de 500 es prudente.
      const chunkSize = 500;
      for (let i = 0; i < asignacionesFinales.length; i += chunkSize) {
        ...
        if (assignError) {
          logger.error(`Error en lote de asignación ${i}-${i + chunkSize}`, assignError);
          // Opcionalmente podrías decidir si continuar o frenar todo
        }
      ...
      return {
        success: true,
        message: `Importación masiva completada: ${rows.length} filas procesadas con protocolos de seguridad.`
      };
    } catch (error) {
      logger.error('Error crítico durante la subida de datos masivos', error);
      throw error;
    }
  }
};
